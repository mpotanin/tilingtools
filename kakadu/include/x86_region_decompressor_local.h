...
inline bool
  simd_horz_resampling_float(int length, kdu_line_buf *src,
                             kdu_line_buf *dst, kdu_uint32 phase,
                             kdu_uint32 num, kdu_uint32 den, int pshift,
  ...
  /* This function implements disciplined horizontal resampling for floating
     ...
     signed integers.  Also 0 <= `phase' < den.  We exploit these bounds to
     keep all our in-loop phase manipulations in 32-bit arithmetic, which
     should speed things up a little if we happen to be running in a 32-bit
     architecture. */
{
#ifdef KDU_NO_SSE
  return false;
...
/*****************************************************************************/
/* STATIC                 ssse3_hshuf_2tap_expand_float                      */
/*****************************************************************************/

#ifndef KDU_NO_SSSE3
static void
  ssse3_hshuf_2tap_expand_float(int length, float *src, float *dst,
                                kdu_uint32 phase, kdu_uint32 num,
                                kdu_uint32 den, int pshift, void **kernels,
                                int kernel_len, int blend_vecs)
{
  int off = (1<<pshift)>>1;
  kdu_int64 num_x4 = ((kdu_int64) num) << 2; // Possible ovfl without 64 bits
  int min_adj = (int)(num_x4/den); // Minimum value of adj=[(phase+num_x4)/den]
    ...
    { // Sufficient to displace `ival0' in order to get `ival1'
      for (; length > 0; length-=4, dp++)
        { 
          ...
          __m128 val0, val1;
          val0 = _mm_mul_ps(_mm_castsi128_ps(ival0),((__m128 *)kern)[0]);
          val1 = _mm_mul_ps(_mm_castsi128_ps(ival1),((__m128 *)kern)[1]);
...
#  define SSSE3_SET_FLOAT_SIMD_HSHUF_2TAP_EXPAND_FUNC(_tgt,_vlen,_bv) \
     if (kdu_mmx_level >= 4) \
       { _tgt=ssse3_hshuf_2tap_expand_float; _vlen=4; _bv=2; }
#else // !KDU_NO_SSSE3
#  define SSSE3_SET_FLOAT_SIMD_HSHUF_2TAP_EXPAND_FUNC(_tgt,_vlen,_bv)
#endif

#define KDRD_SET_FLOAT_SIMD_HSHUF_EXPAND_FUNC(_klen,_efac,_func,_vlen,_bv) \
  { \
    if ((_klen == 2) && (_efac > 1.0F)) \
      SSSE3_SET_FLOAT_SIMD_HSHUF_2TAP_EXPAND_FUNC(_func,_vlen,_bv); \
  }


/*****************************************************************************/
/* STATIC                 ssse3_hshuf_2tap_expand_fix16                      */
/*****************************************************************************/

#ifndef KDU_NO_SSSE3
static void
  ssse3_hshuf_2tap_expand_fix16(int length, kdu_int16 *src, kdu_int16 *dst,
                                kdu_uint32 phase, kdu_uint32 num,
                                kdu_uint32 den, int pshift, void **kernels,
                                int kernel_len, int blend_vecs)
{
...
#  define SSSE3_SET_FIX16_SIMD_HSHUF_2TAP_EXPAND_FUNC(_efac,_tgt,_vlen,_bv) \
     { \
       if (kdu_mmx_level >= 4) \
         { _tgt=ssse3_hshuf_2tap_expand_fix16; _vlen=8; \
           _bv=(_efac > (7.1F/6.0F))?1:2; } \
     }
#else // !KDU_NO_SSSE3
#  define SSSE3_SET_FIX16_SIMD_HSHUF_2TAP_EXPAND_FUNC(_efac,_tgt,_vlen,_bv)
#endif
...
/*****************************************************************************/
/* STATIC                ssse3_hshuf_6tap_resample_fix16                     */
/*****************************************************************************/

#ifndef KDU_NO_SSSE3
static void
  ssse3_hshuf_6tap_resample_fix16(int length, kdu_int16 *src, kdu_int16 *dst,
                                kdu_uint32 phase, kdu_uint32 num,
                                kdu_uint32 den, int pshift, void **kernels,
                                int kernel_len, int blend_vecs)
{
...
#  define SSSE3_SET_FIX16_SIMD_HSHUF_6TAP_RESAMPLE_FUNC(_efac,_tgt,_vlen,_bv) \
     { \
       int tmp_bv = 2; \
       while (7.1f > ((10+8*(tmp_bv-2))*_efac)) tmp_bv++; \
       if ((kdu_mmx_level >= 4) && (tmp_bv <= 3)) \
         { _tgt=ssse3_hshuf_6tap_resample_fix16; _vlen=8; _bv=tmp_bv; } \
     }
#else // !KDU_NO_SSSE3
#  define SSSE3_SET_FIX16_SIMD_HSHUF_6TAP_RESAMPLE_FUNC(_efac,_tgt,_vlen,_bv)
#endif
...
#define KDRD_SET_FIX16_SIMD_HSHUF_EXPAND_FUNC(_klen,_efac,_func,_vlen,_bv) \
  { \
    if ((_klen == 2) && (_efac > 1.0F)) \
      { \
        SSSE3_SET_FIX16_SIMD_HSHUF_2TAP_EXPAND_FUNC(_efac,_func,_vlen,_bv); \
      } \
    else if (_klen == 6) \
      { \
        SSSE3_SET_FIX16_SIMD_HSHUF_6TAP_RESAMPLE_FUNC(_efac,_func,_vlen,_bv); \
      } \
  }
