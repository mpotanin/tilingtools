...
namespace ttx
{
	...
	class RasterBuffer
	{
		public:
			...
			bool CreateBuffer(int num_bands,
								...
								GDALColorTable* p_color_table = 0
								);
			//RasterBuffer* Clone();
			...
			bool InitByValue(float value);

			void* GetPixelDataBlock(int left, int top, int w, int h);
			bool SetPixelDataBlock(int left, int top, int w, int h, void* p_pixel_data_block, 
										int band_min = -1, int band_max = -1);
			void*	ZoomOut(bool use_nearest_resampling = false);

			//ToDo
			//bool CreateAlphaBandByNDV(int nNDV = 0);
	...
	protected:
		...
		template <typename T> bool InitByValue(T type, float value);
		...
		template <typename T> void* ZoomOut(T type, bool use_nearest_resampling = false);
		template <typename T> bool ScaleDataTo8Bit(T type, double* p_scales, double* p_offsets);
		template <typename T> int* GetAllPixelsAsInt(T type);
