  ...
  public: // Member functions
    ...
    KDU_AUX_EXPORT bool finish();
      /* [SYNOPSIS]
           ...
           [//]
           In the case of multi-threaded processing (non-NULL `env' argument
           passed to `start'), after this function returns there is
           guaranteed to be no ongoing processing associated with the
           tile processing engines that were created by this object.  However,
           it is possible that the codestream itself still has ongoing
           background processing.  For this reason, you should remember to
           call `kdu_thread_env::cs_terminate' prior to
           `kdu_codestream::destroy', regardless of whether the present
    ...
    KDU_AUX_EXPORT bool
      ...
      /* [SYNOPSIS]
           ...
           the internal machinery to recognize when it can syhnthesize an
