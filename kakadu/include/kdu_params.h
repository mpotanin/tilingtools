...
#define Sprofile "Sprofile" // One record x
                            ...
                            //   CINEMA2S=7,CINEMA4S=8,CINEMASS=9)"
...
#define Sbroadcast "Sbroadcast" // One record x 
                                // "(single=0,multi=1)(irrev=0,rev=1)"
...
class siz_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `Sbroadcast';
          ...
          level, which is currently required to lie in the range 1 through 7.
          The profile level is concerned with the bit-rate and sampling rate
          of a compressed video stream, as described in an amendment to
...
class org_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `ORGtparts'; Pattern = "[R=1|L=2|C=4]" --
          ...
          more of the flags, `ORGtparts_R', `ORGtparts_L' and
          `ORGtpargs_C'.  If the `ORGtparts_R' flag is supplied, tile-parts
          will be introduced as necessary to ensure that each tile-part
          consists of packets from only one resolution level.  If
          `ORGtparts_L' is supplied, tile-parts are introduced as necessary
          to ensure that each tile-part consists of packets from only one
          quality layer.  Similarly, if `ORGtparts_C' is supplied,
