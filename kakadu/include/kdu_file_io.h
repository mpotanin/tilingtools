...
class kdu_simple_buffer_target;
...
/*****************************************************************************/
class kdu_simple_buffer_target : public kdu_compressed_target {
 
  ///*
  public:
    kdu_simple_buffer_target(int buffer_max_size = 10000000) 
    { 
      can_strip_tail_=false;
      cur_pos_ = 0;
      p_buffer_data_ = new kdu_byte[buffer_max_size];
    }

    ~kdu_simple_buffer_target() 
    { 
      close();
    }
    ...
    virtual bool write(const kdu_byte *buf, int num_bytes)
      { 
        ...
        if (num_bytes > 0)
          {
            memcpy(&p_buffer_data_[cur_pos_],buf,num_bytes);//fwrite(buf,1,(size_t) write_bytes,file);
            cur_pos_ += num_bytes;
          }
        can_strip_tail_ = false;
        return true;
    ...
    bool close()
      { 
        if (p_buffer_data_ != NULL)
          delete[](p_buffer_data_);
        p_buffer_data_ = NULL;
        cur_pos_ = 0;
        can_strip_tail_ = false;
        return true;
  ...
  public: // Data
    kdu_byte *p_buffer_data_;
    kdu_long restore_pos_; // -ve if not in a rewrite
    kdu_long cur_pos_; // Current position in file
    bool can_strip_tail_; // If file is open for update
    //kdu_long max_size;
    //*/
};
...
class kdu_simple_buffer_source : public kdu_compressed_source {
  public:
    kdu_simple_buffer_source (void *p_data, int size)
    {
      buffer_size_=size;
      p_buffer_data_ = new kdu_byte[buffer_size_];
      memcpy(p_buffer_data_,p_data,buffer_size_);
      cur_pos_=0;
    ...
    {
      buffer_size_=0;
      if(!p_buffer_data_) delete[]p_buffer_data_;
      p_buffer_data_=NULL;
      cur_pos_=0;
      return true; 
    }
    ...
    {
      if (cur_pos_+num_bytes>buffer_size_) return 0;
      memcpy(buf,&p_buffer_data_[cur_pos_],num_bytes);      
      cur_pos_+=num_bytes;
    };
    ...
    //ToCheck
    kdu_byte *access_memory(kdu_long &pos, kdu_byte * &lim)
    { 
      return NULL;
      //lim = &p_buffer_data_[buffer_size_];
      //pos = cur_pos_;
      //return &p_buffer_data_[buffer_size_]; 
    }
     
   
    
public:
  kdu_long  buffer_size_;
  kdu_byte  *p_buffer_data_;
  kdu_long  cur_pos_;
 };
 */
...
class kdu_simple_buffer_source : public kdu_simple_file_source {
  public: 
    kdu_simple_buffer_source(void *p_data, int size) { p_data_ = NULL;
        cur_pos_=0;
        size_=0;
        open(p_data,size);
    }
    ~kdu_simple_buffer_source() { close(); }
    bool exists() { return (p_data_ != NULL); }
    bool open(void *p_data, int size)
      {
        close();
        if ((p_data==NULL) || (size<=0)) return false;
        cur_pos_ = 0;
        size_=size;
        p_data_ = new BYTE[size_];
        memcpy(p_data_,p_data,size_);
    ...
    virtual int read(kdu_byte *buf, int num_bytes)
      { 
        if (cur_pos_+num_bytes<=size_)
        {
          memcpy(buf,&((BYTE*)p_data_)[cur_pos_],num_bytes);
          cur_pos_+=num_bytes;
        }
        else if (cur_pos_<size_)
        {
          memcpy(buf,&((BYTE*)p_data_)[cur_pos_],(num_bytes=size_-cur_pos_));
          cur_pos_=size_;
        }
        else num_bytes=0;
